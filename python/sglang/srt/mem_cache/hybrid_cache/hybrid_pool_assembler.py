...
## REMNANT (packed-aware c4 host mirror)
import threading as _remnant_threading  # noqa: E402
import psutil as _remnant_psutil  # noqa: E402
...
class RemnantPackedHostPool(DeepSeekV4PagedHostPool):
    ...
    def __init__(
        ...
        *,
        ...
        self.dtype = _remnant_torch.uint8
        self.device = "cpu"
        ...
        self.lock = _remnant_threading.RLock()
        ...
        for layer in range(self.layer_num):
            ...
            hv = _remnant_torch.zeros(
                (num_host_pages,) + tuple(vals.shape[1:]),
                dtype=_remnant_torch.uint8,
                device="cpu",
                pin_memory=True,
            )
            hb = _remnant_torch.zeros(
                (num_host_pages,) + tuple(bms.shape[1:]),
                dtype=_remnant_torch.uint64,
                device="cpu",
                pin_memory=True,
            )
            hs = _remnant_torch.zeros(
        ...
        host_mem = _remnant_psutil.virtual_memory()
